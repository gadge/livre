...
if (cluster.isMaster) {
  ...
  const pool = new ProcessPool('packages/word-count/test/strategies/clusterPool/wordCountWorker.js', 16, 8)
  const files = fs.readdirSync(SRC)
  for (let file of files) {
    pool.enqueue({ from: M, to: WORKER, code: 'word-count', data: { src: SRC, file } })
  }
